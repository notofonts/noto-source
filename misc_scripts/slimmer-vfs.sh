...
#
if (( $# == 0 )) then echo usage: "slimmer-vfs <variable-ttf-directory>"; exit 1; fi
if ([ ! -d $1 ]) then echo "Directory $1 does not exist"; exit 1; fi
ls $1/Noto*-VF.ttf 1>/dev/null 2>&1 || exit 1
# cd Noto variable font directory
cd $1
rm -r slim slim-drop slim-no-wdth
mkdir slim slim-drop slim-no-wdth
for i in Noto*.ttf ; do echo "====== dropping wdth axis from $i"; fonttools varLib.instancer -o slim-drop/$i $i wdth=drop wght=400:700 ; done
for i in Noto*.ttf ; do echo "====== restricting wght to 400:700 in $i"; fonttools varLib.instancer -o slim-no-wdth/$i $i wght=400:700 ; done
cp slim-drop/Noto*-VF.ttf slim/
